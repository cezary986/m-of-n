...
public class ClassificationExpertFinder extends ClassificationFinder implements IExpertFinder {
			...
			Set<Integer> uncoveredPositives)
	{
		...
		if ((isRuleEmpty && knowledge.isInduceUsingPreferred()) ||
			...
			do {
				ConditionBase condition = induceCondition(rule, dataset, uncoveredPositives, covered, localAllowed, rule.getCoveredPositives());
				carryOn = tryAddCondition(rule, null, condition, dataset, covered, conditionCovered);
				// fixme: we are not sure if condition was added
				if (carryOn && condition instanceof ElementaryCondition) {
					// TODO consider attributes conditions
					ElementaryCondition elementaryCondition = (ElementaryCondition) condition;
					knowledge.getPreferredAttributes((int)classId).remove(elementaryCondition.getAttribute());
					localAllowed.remove(dataset.getAttributes().get(elementaryCondition.getAttribute()));
					used.add(dataset.getAttributes().get(elementaryCondition.getAttribute()));
	...
	protected boolean checkCandidate(ConditionBase cnd, double classId, double totalPositives, double newlyCoveredPositives) {
		...
		if (cnd instanceof ElementaryCondition) {
			ElementaryCondition elementaryCondition = (ElementaryCondition) cnd;
			isForbidden = knowledge.isForbidden(elementaryCondition.getAttribute(), elementaryCondition.getValueSet(), (int)classId);
		}
