...
public class RegressionExpertFinder extends RegressionFinder implements IExpertFinder {
	...
	public void adjust(
			...
			for (ConditionBase cnd : expertPremise.getSubconditions()) {
				...
				if (ec.isAdjustable()) {
					...
					ConditionBase newCondition = induceCondition(
			...
			Set<Integer> uncovered)
	{
		...
		if ((isRuleEmpty && knowledge.isInduceUsingPreferred()) ||
			...
			do {
				ConditionBase condition = induceCondition(rule, dataset, uncovered, covered, localAllowed);
					
				if (condition != null && condition instanceof ElementaryCondition) {
					...
					knowledge.getPreferredAttributes().remove(elementaryCondition.getAttribute());
