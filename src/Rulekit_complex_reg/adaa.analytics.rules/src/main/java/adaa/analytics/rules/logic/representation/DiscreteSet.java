 ...
 *
 */
public class DiscreteSet implements IValueSet, Serializable {
	 ...
	 * Creates empty set.
	 */
	public DiscreteSet(Set<Double> values,  List<String> mapping){
		super();
		this.values = values;
	...
	@Override
	public void negate() {
		throw new UnsupportedOperationException("Negation is not supported for this value set");
	}
	...
	public IValueSet getIntersection(IValueSet set) {
		...
		DiscreteSet ret = new DiscreteSet(this.values, this.mapping);
	...
	public List<IValueSet> getDifference(IValueSet set) {
		...
		if (!this.intersects(set) || set instanceof AnyValueSet) {
			...
			DiscreteSet s = new DiscreteSet(this.values, this.mapping);
