...
public class ElementaryCondition extends ConditionBase {
     ...
     * Creates empty condition.
     */
    protected ElementaryCondition(boolean negated) {
     ...
     * @param valueSet  Value set.
     */
    public ElementaryCondition(String attribute, IValueSet valueSet, boolean negated) {
        this.attribute = attribute;
        this.valueSet = valueSet;
        this.negated = negated;

        if (!(valueSet instanceof SingletonSet) && !(valueSet instanceof Interval)) {
            throw new IllegalArgumentException("Negations currently are only supported for SingletonSet");
        }
        if (negated) {
            this.valueSet.negate();
        }
    }
    ...
    protected boolean internalEvaluate(Example ex) {
        ...
        return this.negated ? !valueSet.contains(v) : valueSet.contains(v);
    }
    ...
    protected void internalEvaluate(ExampleSet set, Set<Integer> outIndices) {
        ...
        for (int id = 0; id < set.size(); ++id) {
            ...
            if (this.negated != valueSet.contains(v)) {
                outIndices.add(id);
            }
        }
    }
    ...
    public boolean equals(Object obj) {
        if (obj == this) {
            ...
            if (ref != null) {
                return this.valueSet.equals(ref.getValueSet()) && this.attribute.equals(ref.getAttribute()) && negated == ref.negated;
            } else {
                return false;
            }
        }
    ...
    public ElementaryCondition intersect(ElementaryCondition other) {
        if (this.valueSet.intersects(other.getValueSet())) {
            IValueSet intersection = this.valueSet.getIntersection(other.getValueSet());
            return intersection == null ? null : new ElementaryCondition(attribute,intersection);
        } else {
            return null;
        }
    }
