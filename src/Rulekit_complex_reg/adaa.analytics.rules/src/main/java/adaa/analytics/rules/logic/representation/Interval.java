...
public class Interval implements IValueSet, Serializable {
	...
	/** Flag indicating if interval was induced using interval complex conditions*/
	...
	public void setIsRealInterval(boolean realInterval) {
	...
	public boolean equals(Object obj) {
		...
		return left == ival.left && right == ival.right && leftClosed == ival.leftClosed && rightClosed == ival.rightClosed && negated == ival.negated;
	}
	...
	public boolean intersects(IValueSet set) {
		if (this.negated) {
			if (set instanceof  Interval) {
				Interval ds = (Interval) set;
				Interval leftInterval = new Interval(Double.NEGATIVE_INFINITY, this.left,false, false);
				Interval rightInterval = new Interval(this.right, Double.POSITIVE_INFINITY, true, false);
				return leftInterval.intersects(ds) || rightInterval.intersects(ds);
			}
		}
	...
	public int hashCode() {
		...
		builder.append(leftClosed).append(rightClosed).append(left).append(right).append(negated);
