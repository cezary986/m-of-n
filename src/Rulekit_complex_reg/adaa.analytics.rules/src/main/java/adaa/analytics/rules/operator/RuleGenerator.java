...
public class RuleGenerator extends AbstractLearner implements OperatorI18N {
	...
	public static final String PARAMETER_DISCRETE_SET_CONDITIONS_ENABLED = "discrete_set_conditions_enabled";
	/** If set to true RuleKit will try to induce negated elementary conditions **/
	public static final String PARAMETER_NEGATED_CONDITIONS_ENABLED = "negated_conditions_enabled";
	/** If set to true RuleKit will try to induce conditions like: attr = <value_1, value_2) **/
	public static final String PARAMETER_INTERVALS_CONDITIONS_ENABLED = "intervals_conditions_enabled";
	...
	public static final String PARAMETER_NUMERICAL_ATTRIBUTES_CONDITIONS_ENABLED = "numerical_attributes_conditions_enabled";
	/** If set to true RuleKit will try to induce conditions like: attr_1 = attr_2 for nominal attributes */
	public static final String PARAMETER_DISCRETE_ATTRIBUTES_CONDITIONS_ENABLED = "nominal_attributes_conditions_enabled";

	/** If set to true RuleKit will try to induce inner alternative conditions */
	public static final String PARAMETER_INNER_ALTERNATIVES_ENABLED = "inner_alternatives_enabled";
	/** Number of elements in beam during beam search finding inner alternatives conditions  */
	public static final String PARAMETER_INNER_ALTERNATIVES_SEARCH_BEAM_SIZE = "inner_alternatives_search_beam_size";
	/** Max number of beam search iterations while finding inner alternatives conditions  */
	public static final String PARAMETER_INNER_ALTERNATIVES_MAX_SEARCH_ITERATIONS = "inner_alternatives_max_search_iterations";
	...
	public Model learn(ExampleSet exampleSet) throws OperatorException {
		...
		try {
			...
			params.setDiscreteSetConditionsEnabled(getParameterAsBoolean(PARAMETER_DISCRETE_SET_CONDITIONS_ENABLED));
			params.setNegatedConditionsEnabled(getParameterAsBoolean(PARAMETER_NEGATED_CONDITIONS_ENABLED));
			params.setAttributesIntervalsEnabled(getParameterAsBoolean(PARAMETER_INTERVALS_CONDITIONS_ENABLED));
			params.setAttributesConditionsEnabled(getParameterAsBoolean(PARAMETER_NUMERICAL_ATTRIBUTES_CONDITIONS_ENABLED));
			params.setDiscreteAttributesConditionsEnabled(getParameterAsBoolean(PARAMETER_DISCRETE_ATTRIBUTES_CONDITIONS_ENABLED));

			params.setInnerAlternativesEnabled(getParameterAsBoolean(PARAMETER_INNER_ALTERNATIVES_ENABLED));
			params.setInnerAlternativesMaxSearchIterations(getParameterAsInt(PARAMETER_INNER_ALTERNATIVES_MAX_SEARCH_ITERATIONS));
			params.setInnerAlternativesSearchBeamSize(getParameterAsInt(PARAMETER_INNER_ALTERNATIVES_SEARCH_BEAM_SIZE));
	...
	public Model learn(ExampleSet exampleSet, HashMap<String, HashSet<String>> attributesMappings) throws OperatorException {
		Model model = null;
		...
		try {
			...
			if (exampleSet.getAttributes().findRoleBySpecialName(SurvivalRule.SURVIVAL_TIME_ROLE) != null) {
				...
				params.setVotingMeasure(new LogRank());
				finder = new SurvivalLogRankFinder(params);
				((SurvivalLogRankFinder) finder).attributesMappings = attributesMappings;
				snc = new SurvivalLogRankSnC((SurvivalLogRankFinder)finder, params);
				//	} else {
				//		ClassificationFinder finder = new ClassificationFinder(params);
				//		snc = new SurvivalClassificationSnC(finder, params);
				...
				// regression problem
				finder = new RegressionFinder(params);
				((RegressionFinder) finder).attributesMappings = attributesMappings;
				snc = new RegressionSnC((RegressionFinder) finder, params);
			} else {
				// classification problem
				finder = new ClassificationFinder(params);
				((ClassificationFinder) finder).attributesMappings = attributesMappings;
				snc = new ClassificationSnC((ClassificationFinder) finder, params);
			}
