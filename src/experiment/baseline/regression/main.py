...
import sys
sys.path.append('../../')
sys.path.append('../../m_of_n_regression')
sys.path.append('../../../')
from m_of_n_regression.steps.load_data import load_data as _load_data  # nopep8
from m_of_n_regression.steps.produce_results import _string_supporting_mean, _string_supporting_std  # nopep8
import m_of_n_regression.config as config  # nopep8
...
def evaluate_model(
    ...
    RMSE_test = np.sqrt(metrics.mean_squared_error(y_test, prediction_test))
    MAE_test = metrics.mean_absolute_error(y_test, prediction_test)
    MAPE_test = metrics.mean_absolute_percentage_error(y_test, prediction_test)

    RMSE_train = np.sqrt(metrics.mean_squared_error(y_train, prediction_train))
    ...
    return pd.DataFrame([{
        ...
        'MAE (test)': MAE_test,
        'MAPE (test)': MAPE_test,
        'rRMSE (test)': RMSE_test / np.mean(y_test),
        'rMAE (test)': MAE_test / np.mean(y_test),
        'rMAPE (test)': MAPE_test / 100,
        'maxError (test)': metrics.max_error(y_test, prediction_test),
        'R2 (test)': metrics.r2_score(y_test, prediction_test),

        'RMSE (train)': RMSE_train,
        'MAE (train)': MAE_train,
        'MAPE (train)': MAPE_train,
        'rRMSE (train)': RMSE_train / np.mean(y_train),
        'rMAE (train)': MAE_train / np.mean(y_train),
        'rMAPE (train)': MAPE_train / 100,
        'maxError (train)': metrics.max_error(y_train, prediction_train),
        'R2 (train)': metrics.r2_score(y_train, prediction_train),
...
def decision_tree_regression_baseline(
    dataset_name: str
):
    store = Store()
    ...
    for fold_index in range(1, 11):
        data = load_data(
            ...
            logger=decision_tree_regression_baseline.logger,
        )
...
if __name__ == '__main__':
    decision_tree_regression_baseline([
