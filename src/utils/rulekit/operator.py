...
from .exceptions import JavaBackendException
import numpy as np
import pandas as pd
...
class BaseOperator:
    ...
    def fit(self, values: Data, labels: Data, survival_time_attribute: str = None, attributes_mappings: Dict[str, List[str]] = None) -> Any:
        try:
            example_set = create_example_set(
                values, labels, survival_time_attribute=survival_time_attribute)
            if attributes_mappings is None:
                java_model = self._rule_generator.learn(example_set)
            else:
                java_model = self._rule_generator.learn(example_set, attributes_mappings)
            self.model = RuleSet(java_model)
            return self.model
        except Exception as error:
            if isinstance(error, JException):
                raise JavaBackendException(java_exception=error)
            else:
                raise error
